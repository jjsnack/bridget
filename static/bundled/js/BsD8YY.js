...
const PRESETS = {
  mobile: {
    ...
    collectionTileGap: "4rem",
    // Increased from 2rem for more vertical space
...
function getPresetFromRatio(ratio) {
...
function applyCSSVariables(config, presetName) {
...
function ViewportProvider(props) {
  ...
  const [config, setConfig] = createSignal(PRESETS[initialPreset]);
  ...
  const contextValue = [preset, config, ratio];
